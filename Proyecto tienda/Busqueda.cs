...
namespace Proyecto_tienda
{
    ...
    class CarritoRelacion
    {
        public string comando(string dato)
        {
            ...
            if (dato == "")
            {
                comando = "select cliente.nombre as cliente_nombre,cliente.apellido as cliente_apellido,producto.Marcha as Marca_Seleccionada,producto.Nom_Modelo as Modelo_Seleccionado,rastreo.FECHA_SAL as Fecha_de_Salida_del_Paquete,rastreo.FECHA_LLEGADA as Fecha_de_Llegada_del_Paquete,envio.Direccion_Envio as DIRECCION_ENVIO,(envio.costo+catalogo.precio) as Total_a_Pagar from Cliente,Producto,rastreo,carrito_relacion,envio,catalogo Where producto.Cod_Producto= carrito_relacion.Cod_Producto AND cliente.Cod_Cliente = carrito_relacion.Cod_Cliente AND rastreo.Num_Envio= carrito_relacion.Num_Envio AND ENVIO.Num_Envio= carrito_relacion.Num_Envio AND CATALOGO.Cod_Producto= carrito_relacion.Cod_Producto AND CATALOGO.ID=carrito_relacion.ID_catalogo ORDER BY cliente.nombre ";
            }
            else
            {
                comando= "select cliente.nombre as cliente_nombre,cliente.apellido as cliente_apellido,producto.Marcha as Marca_Seleccionada,producto.Nom_Modelo as Modelo_Seleccionado,rastreo.FECHA_SAL as Fecha_de_Salida_del_Paquete,rastreo.FECHA_LLEGADA as Fecha_de_Llegada_del_Paquete,envio.Direccion_Envio as DIRECCION_ENVIO,(envio.costo+catalogo.precio) as Total_a_Pagar from Cliente,Producto,rastreo,carrito_relacion,envio,catalogo Where producto.Cod_Producto= carrito_relacion.Cod_Producto AND cliente.Cod_Cliente = carrito_relacion.Cod_Cliente AND rastreo.Num_Envio= carrito_relacion.Num_Envio AND ENVIO.Num_Envio= carrito_relacion.Num_Envio AND CATALOGO.Cod_Producto= carrito_relacion.Cod_Producto  AND CATALOGO.ID=carrito_relacion.ID_catalogo AND (cliente.nombre='" + dato+ "' OR cliente.apellido='" + dato + "' OR cliente.apellido='" + dato + "' OR producto.Marcha='" + dato + "' OR producto.Nom_Modelo='" + dato + "'  OR cliente.nombre='" + dato + "')";
            }
            return comando;
        }
    }
